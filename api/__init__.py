...
class FreqTable(object):
    ...
    def get_by_chr_pos(self, chr, pos):
        ...
        if data_files[self.dataset]['multiple_chr']:
            vcf_filename = data_files[self.dataset]['data_dir'] + data_files[self.dataset][
                'file_base'] + str(chr) + data_files[self.dataset]['file_end']
        else:
            vcf_filename = data_files[self.dataset][
                'data_dir'] + data_files[self.dataset]['file']

        tabix_snp_pos = str(chr) + ':' + str(pos) + '-' + str(pos)
        tabix_command = '/home/josephhmarcus/bin/tabix-0.2.6/tabix ' + \
            vcf_filename + ' ' + tabix_snp_pos
        proc = subprocess.Popen(
            tabix_command, stdout=subprocess.PIPE, shell=True)
    ...
    def get_by_rsID(self, rsID):
        '''
        '''
        rest_command = "curl 'http://grch37.rest.ensembl.org/variation/human/" + \
            rsID + "?' -H 'Content-type:application/json'"
        proc = subprocess.Popen(
            rest_command, stdout=subprocess.PIPE, shell=True)
        (out, err) = proc.communicate()
        ...
        if data_files[self.dataset]['multiple_chr']:
            ...
            tabix_snp_pos = lo_list[0][0][3:] + ':' + \
                str(lo_list[0][1]) + '-' + str(lo_list[0][1])
            vcf_filename = data_files[self.dataset][
                'data_dir'] + data_files[self.dataset]['file']
    ...
    def get_random(self):
        ...
        if data_files[self.dataset]['multiple_chr']:
            ...
            snp_filename = data_files[self.dataset]['data_dir'] + data_files[self.dataset][
                'file_base'] + str(chr) + data_files[self.dataset]['random_end']
        else:
            snp_filename = data_files[self.dataset][
                'data_dir'] + data_files[self.dataset]['random_file']
        snp_proc = subprocess.Popen(
            'shuf -n 1 ' + snp_filename, stdout=subprocess.PIPE, shell=True)
    ...
    def define_freqscale(self, json_data):
        ...
        if maxfreq < 0.001:
            for i in range(0, len(json_data)):
                json_data[i]['freq'] = [json_data[i]['rawfreq']
                                        * 1000, 1 - json_data[i]['rawfreq'] * 1000]
                json_data[i]['freqscale'] = 0.001
        elif maxfreq < 0.01:
            for i in range(0, len(json_data)):
                json_data[i]['freq'] = [json_data[i]['rawfreq']
                                        * 100, 1 - json_data[i]['rawfreq'] * 100]
                json_data[i]['freqscale'] = 0.01
        elif maxfreq < 0.1:
            for i in range(0, len(json_data)):
                json_data[i]['freq'] = [json_data[i]['rawfreq']
                                        * 10, 1 - json_data[i]['rawfreq'] * 10]
                json_data[i]['freqscale'] = 0.1
        else:
            for i in range(0, len(json_data)):
                json_data[i]['freq'] = [json_data[i][
                    'rawfreq'], 1 - json_data[i]['rawfreq']]
